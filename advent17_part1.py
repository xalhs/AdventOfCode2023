...
def map_altering(pattern1 , condition = False):
    ...
    if condition:
        ...
        tracker = [ [  {"L" : [10000 , 10000, 10000] , "R" : [10000 , 10000, 10000] , "U" : [10000 , 10000, 10000] , "D" : [10000 , 10000, 10000] }.copy()  for i in range(len(pattern1))] for j in range(len(pattern1)) ]
    basis = {"L" : [10000 , 10000, 10000] , "R" : [10000 , 10000, 10000] , "U" : [10000 , 10000, 10000] , "D" : [10000 , 10000, 10000] }
    for i in range(2*len(pattern1) -1):
        for j in range(max(i+1-len(pattern1) , 0) , min(i+1 , len(pattern1))):
            ...
            if i ==0 and j ==0:
                ...
                zero_val = 0
                tracker[j][i-j] = {"L" : [zero_val , zero_val, zero_val] , "R" : [zero_val , zero_val, zero_val] , "U" : [zero_val , zero_val, zero_val] , "D" : [zero_val , zero_val, zero_val] }
                continue
            for direc in basis:
                        ...
                        for direct in basis:
                            if direc == direct or direct == opposite_dir[direc]:
                                ...
                                if is_coord_in(other_dir):
                                    for neu_val in  tracker[other_dir[0]][other_dir[1]][direct]:
                                        if neu_val < mini:
                                            mini = neu_val
...
def find_path(tracker):
    ...
    while cur_cor != [0,0]:
        ...
        for thing in aval_dir:
            for k,val in enumerate(tracker[cur_cor[0]][cur_cor[1]][thing]):
                if val < mini:
                    mini = val
                    cards = [thing , k+1]
...
for direction in dir_list:
    for k in range(len(tracker[length][length][direction])):
